// src/commands/info.js - Info Command
...
module.exports = {
    ...
    async createOverviewEmbed() {
        ...
        return new EmbedBuilder()
            ...
            .addFields([
                ...
                { name: '💰 Economy', value: 'Earn berries automatically based on your CP to pull more fruits', inline: true },
                { name: '🎮 Key Features', value: '• Animated gacha pulls\n• Duplicate system (+1% CP per dupe)\n• Element counter system\n• Role-based leveling\n• Automatic income', inline: false },
                ...
                { name: '🚀 Getting Started', value: 'Use `/pull` to get your first Devil Fruit!\nUse `/income` to collect berries every hour!', inline: false }
            ])
    ...
    async createFruitsEmbed() {
        return new EmbedBuilder()
            ...
            .addFields([
                ...
                { name: '⚡ CP Calculation', value: 'Total CP = Base CP × (Sum of all fruit multipliers + duplicate bonuses)', inline: false },
    ...
    async createRatesEmbed() {
        ...
        return new EmbedBuilder()
            ...
            .addFields([
                ...
                { name: '💰 Pull Cost', value: '1,000 berries per pull', inline: true },
    ...
    async createLevelsEmbed() {
        return new EmbedBuilder()
            ...
            .addFields([
                ...
                { name: '💎 Base CP', value: 'Your base CP from your level is multiplied by all your Devil Fruit multipliers', inline: false },
    ...
    async createEconomyEmbed() {
        return new EmbedBuilder()
            ...
            .addFields([
                { name: '💵 Berry Income', value: 'Earn berries automatically every 10 minutes based on your total CP', inline: false },
                { name: '📊 Income Formula', value: 'Hourly Income = 50 base + (Total CP × 0.1)', inline: false },
                { name: '⏰ Collection', value: 'Use `/income` to collect up to 24 hours of accumulated berries', inline: false },
                { name: '🛒 Spending', value: 'Use berries to pull Devil Fruits (1,000 berries per pull)', inline: false },
                { name: '🔄 Automatic Income', value: 'Income is generated every 10 minutes automatically in the background', inline: false },
                { name: '💡 Tips', value: '• Higher CP = more berries per hour\n• Collect income regularly (max 24 hours)\n• Save berries for multiple pulls\n• Focus on getting more fruits to increase CP', inline: false }
            ])
    ...
    async createCommandsEmbed() {
        return new EmbedBuilder()
            ...
            .addFields([
                { name: '🎮 Core Commands', value: '`/pull` - Pull a Devil Fruit (1,000 berries)\n`/income` - Collect your berry income\n`/collection` - View your Devil Fruit collection\n`/stats` - View your pirate stats', inline: false },
